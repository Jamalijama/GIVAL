...
# 构建数据集生成器
vocab = 'vocab.txt'
data = './data/try_Spike_sample_token_onlywith_HMM_seg_RBD_variant1000.txt'
max_len = 257
...
word_dim=768
gene = 'Spike_RBD'
method_name = 'RBD1000_transformer_768_2e-4_2048'
path1 = '768_2e-4_2048/'
'''
#padding
...
input_array0 = np.load('try_Spike_sample_token_onlywith_HMM_seg_RBD_variant1000_38w_256cut_HMM.npy', allow_pickle=True)
method_name = 'Spike_seg_RBD_256cut_HMM_38w'
...
df_all_information = pd.read_csv('./embedding_eval/RBD/Spike_sampled_variant_not_in_training_set.csv')
...
#
df_evaluation.to_csv('./embedding_eval/RBD/'+path1+'df_evaluate_cluster_'+method_name+'_tSNE.csv')
...
df_all_information.to_csv('./embedding_eval/RBD/'+path1+'tSNE_result_with_MiniBatchKMeans_label_'+method_name+'.csv')
...
for a in range(2):
    ...
    plt.savefig('./embedding_eval/RBD/'+path1+'sns_scatterplot_tSNE_' + method_name + '.png', dpi = 300, bbox_inches = 'tight')
np.save('./embedding_eval/RBD/'+path1+'feature_' + method_name + '.npy',np_outputs,allow_pickle=True)
