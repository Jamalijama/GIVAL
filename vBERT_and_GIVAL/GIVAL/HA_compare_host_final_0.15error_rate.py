...
#extract_feature
method_name = gene+'_for_predict_model_compare_HA_host_error_rate'+str(error_rate)
...
#true_label_origin0 = df_csv_origin['Host'].tolist()
...
#df_H3_train = sklearn.utils.shuffle(df_H3_train,random_state=10)
#df_H3_train = df_H3_train.sample(frac=0.8,random_state=10)
...
true_label_origin0 = df_all_information1['Host1'].tolist()
true_label_origin = []
...
label_lst = sorted(set(df_all_information[origin_label]))
label_len_lst = []
for label in label_lst:
    df_label = df_all_information[df_all_information[origin_label]==label]
    print(len(df_label))
    label_len_lst.append(len(df_label))

df_sample = pd.DataFrame()
for label0 in label_lst:
    df_label0 = df_all_information[df_all_information[origin_label]==label0]
...
labels00 = df_sample['Host1'].tolist()
label_num = len(label_lst)
label_num_lst = [l for l in range(label_num)]
#dict_label_num = dict(zip(label_lst,label_num_lst))
...
print('dict_label_num=',dict_label_num)
labels = []
for label in labels00:
    labels.append(int(dict_label_num[label]))
...
cm = confusion_matrix(true_label_origin, pred_lst)
print('final_cm=',cm)
